...
import os
import shutil       
from vlm_cytomorphology_eval.config.dataset_info_and_paths import get_dataset_info, get_result_path, get_vlm_eval_subset_folder_path



def select_images_for_expert_review(number_per_type_and_correctness, vlm_name, dataset_name, task_type='nonstructured'):
    ...
    expert_data_folder_path = get_vlm_eval_subset_folder_path(dataset_name, 'Expert_'+vlm_name)
    
    
    os.makedirs(expert_data_folder_path, exist_ok=True)
    ...
    df_for_expert = pd.DataFrame(columns=['image_name', 'correct_cell_type', 'correctness (1-excellent, 2-good, 3-ok, 4-poor, 5-completely wrong)', 'mistakes', 'LLM_description'])
    df_for_us = pd.DataFrame(columns=['image_name', 'original_image_name', 'ground_truth_cell_type', 'predicted_cell_type', 'expert_cell_type', 'correctness (1-excellent, 2-good, 3-ok, 4-poor, 5-completely wrong)', 'mistakes', 'LLM_description'])

    cell_count=0

    def enter_row(cell_type,correct_wrong):
        nonlocal cell_count
        nonlocal df_for_expert
        nonlocal df_for_us
        ...
        expert_image_name = 'cell_'+str(cell_count)
        
        cell_count += 1

        expert_image_path = os.path.join(expert_data_folder_path, expert_image_name+'.png')

        df_for_expert_row = pd.DataFrame({'image_name': expert_image_name, 'correct_cell_type': '', 'correctness (1-excellent, 2-good, 3-ok, 4-poor, 5-completely wrong)': np.nan, 'mistakes': '', 'LLM_description': LLM_description}, index=[0])

        df_for_us_row = pd.DataFrame({'image_name': expert_image_name, 'original_image_name': original_image_name, 'ground_truth_cell_type': ground_truth_label, 'predicted_cell_type': predicted_label, 'expert_cell_type': '', 'correctness (1-excellent, 2-good, 3-ok, 4-poor, 5-completely wrong)': np.nan, 'mistakes': '', 'LLM_description': LLM_description}, index=[0])


        df_for_expert = pd.concat([df_for_expert, df_for_expert_row], ignore_index=True)
        df_for_us = pd.concat([df_for_us, df_for_us_row], ignore_index=True)

        # Copy image from test folder to expert folder with new name
        try:
            original_image_path = os.path.join(testset_folder_path, original_image_name+'.jpg')
            expert_image_path = os.path.join(expert_data_folder_path, expert_image_name+'.jpg')
            shutil.copy2(original_image_path, expert_image_path)
        except:
            try:
    ...
    df_for_expert.to_excel(os.path.join(expert_data_folder_path, 'expert_review.xlsx'), index=False)
    df_for_us.to_excel(os.path.join(expert_data_folder_path, 'us_complete_data.xlsx'), index=False)
...
select_images_for_expert_review(number_per_type_and_correctness, vlm_name, dataset_name, task_type='nonstructured')
