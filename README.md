...
3. Add a new api inquiry code file to the vlm_models folder on github. Call it <<model_name>>_api.py.
It needs to contain the following functions:
    - <<gpt>>_api_api_visual_inquiry. Takes image_path, prompt_text, vlm_name as inputs and returns the answer and token usage.This function is used to pass a single image and prompt to the VLM model.
    ...
    - <<gpt>>_api_task for every task type that requires more than a single image. It needs to take suitable inputs and return the answer and token usage. See other models for examples.
    ...
    - <<gpt>>_api_text_inquiry. Takes prompt_text, vlm_name as inputs and returns the answer and token usage. This function is used to review the answers generated by the VLM models.

See gpt_api.py for an example.

4. Import the model in the run_api_inquiry.py file. Add the model to the if clauses in run_api_0shot_classification, ..., run_api_task_type,... and run_api_review functions.
